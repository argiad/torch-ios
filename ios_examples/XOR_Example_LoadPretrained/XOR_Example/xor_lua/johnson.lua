...
local function res_block()
  ...
  conv_block:add(backend.SpatialConvolutionMM(128, 128, 3, 3, 1, 1, 0, 0))
...
if params_mode == 'texture' then
  model:add(nn.NoiseFill(3))
end

model:add(pad(4, 4, 4, 4))
model:add(backend.SpatialConvolutionMM(3, 32, 9, 9, 1, 1, 0, 0))
model:add(normalization(32))
model:add(nn.ReLU(true))

-- probably need replication padding here too
model:add(backend.SpatialConvolutionMM(32, 64,  3, 3, 2, 2, 1, 1))
model:add(normalization(64))
model:add(nn.ReLU(true))

-- probably need replication padding here too
model:add(backend.SpatialConvolutionMM(64, 128, 3, 3, 2, 2, 1, 1))
...
model:add(backend.SpatialConvolutionMM(32, 3, 3, 3, 1, 1, 0, 0))

return model:add(nn.TVLoss(params_tv_weight))